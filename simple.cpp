...
double Tsim=0.0;     	//Tiempo de simulaci�n (reloj)
struct Evt *head=NULL;   //Apunta siempre al primer evento de la LEF o a NULL si �sta est� vac�a
...
double Erlang_B(unsigned int s,double a){//s: servidores, a: tr�fico ofrecido
	double IB=1.0;
...
enum EvtTyp evento_actual(){
	...
	Tsim=head->t_evento;                     //Actualiza el tiempo de simulaci�n.		           
    head=head->sig;                          //Ahora head apunta al siguiente evento de la LEF o a Null si la lista e qued� vac�a
	free(p);                                 //Libera memoria cupada por el primer evento de la LEF        
	return evt_inminente;                    //regresa el tipo de evento que se procesar�
}


void programa_evento(enum EvtTyp type, double epoch){
	struct Evt *p; //Apuntador auxiliar para ir recorriendo la LEF
	bool insertado=0;  // insertado=0 indica que a�n no se ha insertado el nuevo evento en la LEF
	...
	newevt->sig=head; //"newevt->sig" apunta a NULL si la LEF est� vac�a o al primer evento de la LEF
	while (insertado==0){
		if(newevt->sig==NULL || (newevt->t_evento < (newevt->sig)->t_evento) ){
			...
			insertado=1;      //Indica que ya se insert� el evento nuevo en la LEF
		}
		else {
...
int main(){
	...
	double Pb=(float)bloqueados/arribos;   //Congesti�n en llamada (simulador)
	double Eb=Erlang_B(s,a);            //Congesti�n en tiempo analitico (Erlang B)
